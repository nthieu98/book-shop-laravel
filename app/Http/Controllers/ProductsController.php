...
class ProductsController extends Controller
{
	...
	public function addProduct(Request $request){
        ...
        // return $categories;
		$categories_drop_down = "<option value=''  disabled placeholder = 'Select' >Select</option>";
		foreach($categories as $cat){
			$categories_drop_down .= "<option value='".$cat->categoryId."'>".$cat->categoryName."</option>";
	...
	public function editProduct(Request $request,$id=null){
		...
		if($request->isMethod('post')){
			...
			Product::where(['productId'=>$id])->update(['categoryId'=>$data['category_id'],'productName'=>$data['product_name'],
				'productQuantity'=>$data['product_quantity'],'productPrice'=>$data['product_price'],'productImage'=>$fileName]);
                
			return redirect()->back()->with('flash_message_success', 'Product has been edited successfully');
		}

		// Get Product Details start //
		$productDetails = Product::where(['productId'=>$id])->first();
		...
		foreach($categories as $cat){
			if($cat->categoryId==$productDetails->categoryId){
				$selected = "selected";
			}else{
				$selected = "";
			}
			$categories_drop_down .= "<option value='".$cat->categoryId."' ".$selected.">".$cat->categoryName."</option>";
			// $sub_categories = Category::where(['categoryId' => $cat->categoryId])->get();
			// foreach($sub_categories as $sub_cat){
	...
	public function viewProducts(Request $request){
		...
		foreach($products as $key => $val){
			$category_name = Category::where(['categoryId' => $val->categoryId])->first();
			$products[$key]->categoryName = $category_name->categoryName;
		}
